...
import type { Message } from "../../types/line/line_types.js";
...
export class LineObs {
	...
	public async uploadMediaByE2EE(options: {
		...
		const edata = new Blob([encryptedData as unknown as BlobPart]);
		...
		if (oType === "image" || oType === "gif" || oType === "video") {
			...
			if (preview) {
				...
				previewEdata = new Blob([enc.encryptedData as unknown as BlobPart]);
			} else {
				previewEdata = edata;
			}
			...
			if (objId !== objId2) {
				throw new InternalError(
					"ObsError",
					"objId not match: " + JSON.stringify(Object.fromEntries(headers as any)),
					{
